...
const Course = React.memo((props: CourseProps) => {
  ...
  const renderTimes = useCallback(() => {
    if (!timesRendered) return null;

    const startStr = fStart.toLocaleTimeString(i18n.language, { hour: "2-digit", minute: "2-digit" });
    const endStr = fEnd.toLocaleTimeString(i18n.language, { hour: "2-digit", minute: "2-digit" });
    ...
    if (i18n.language === "en") {
      return (
        <Stack
          ...
          gap={3}
        >
          <Typography nowrap variant="h5" align="center" style={{ lineHeight: 20, width: 60, fontSize: 13 }} skeleton={skeleton}>
            {startStr}
          </Typography>
          <Typography nowrap variant="body2" color="secondary" align="center" style={{ width: 60, fontSize: 13 }} skeleton={skeleton}>
            {endStr}
          </Typography>
        </Stack>
      );
          ...
          {variant === "primary" && (
            <Stack direction="horizontal" style={{ marginTop: 1, opacity: skeleton ? 0.5 : 1 }}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 5, flex: 1, minWidth: 0 }}>
                ...
                >
                  {teacher ? (() => {
                    const parts = teacher.split(' ').filter(p => p.length > 0);
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 7, marginTop: status.label ? 4 : 0, opacity: skeleton ? 0.5 : 1 }}>
              {status.label && (
                ...
                >
                  <Typography
                    variant="h4"
                    style={[styles.statusLabel, { color: dark ? "#FFFFFF" : color, padding: 0, fontSize: 12 }]}
                    skeleton={skeleton}
                    numberOfLines={1}
                  >
                    {status.label}
