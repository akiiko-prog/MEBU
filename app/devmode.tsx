  ...
  return (
    ...
    >
      ...
      <List>
        ...
        >
          <Typography variant="title">Vider le cache des IDs vus</Typography>
          <Typography variant="caption">Simule l'arrivée de nouvelles notes</Typography>
        </Item>
        <Item
          onPress={async () => {
            try {
              Alert.alert("Lancement...", "Le background refresh se lance.");
              const { runBackgroundRefresh } = await import('@/utils/background/backgroundRefresh');
              const result = await runBackgroundRefresh();
              Alert.alert("Terminé", `Résultat: ${result === 1 ? 'NewData' : result === 2 ? 'NoData' : 'Failed'}\nVérifie les logs et notifications.`);
            } catch (e) {
              Alert.alert("Erreur", String(e));
            }
          }}
        >
          <Typography variant="title">Lancer le Background Refresh</Typography>
          <Typography variant="caption">Exécute le task immédiatement</Typography>
        </Item>
        <Item
          onPress={async () => {
            try {
              ...
              const { runBackgroundRefresh } = await import('@/utils/background/backgroundRefresh');
              const result = await runBackgroundRefresh();
              Alert.alert("✅ Test terminé", `Cache vidé + refresh exécuté.\nRésultat: ${result === 1 ? 'NewData' : result === 2 ? 'NoData' : 'Failed'}\nVérifie les notifications !`);
            } catch (e) {
              Alert.alert("Erreur", String(e));
            }
          }}
        >
          <Typography variant="title">🧪 Test complet (cache + refresh)</Typography>
          <Typography variant="caption">Vide le cache puis lance le refresh</Typography>
        </Item>
        <Item
          onPress={async () => {
            try {
              const { sendNewGradeNotification } = await import('@/utils/notification/alertNotification');
              await sendNewGradeNotification("Mathématiques (test)", "18.5/20");
              Alert.alert("Notification envoyée", "Vérifie le centre de notifications !");
            } catch (e) {
              Alert.alert("Erreur", String(e));
            }
          }}
        >
          <Typography variant="title">🔔 Envoyer une notif test</Typography>
          <Typography variant="caption">Notification de nouvelle note fictive</Typography>
        </Item>
      </List>
