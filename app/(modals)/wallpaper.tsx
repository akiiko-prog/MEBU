...
const CITIES = [
  { id: 'city_lyon', name: 'Lyon', asset: require('@/assets/images/background/lyon.jpg') },
  { id: 'city_paris', name: 'Paris', asset: require('@/assets/images/background/paris.jpg') },
  { id: 'city_rennes', name: 'Rennes', asset: require('@/assets/images/background/rennes.jpg') },
  { id: 'city_strasbourg', name: 'Strasbourg', asset: require('@/assets/images/background/strasbourg.jpg') },
  { id: 'city_toulouse', name: 'Toulouse', asset: require('@/assets/images/background/toulouse.jpg') },
]
...
const WallpaperModal = () => {
  ...
  const downloadCity = async (cityItem: typeof CITIES[0]) => {
    const fileName = `${cityItem.id}.jpg`;
    const wallpaperFile = new File(wallpaperDirectory, fileName);
    ...
    setCurrentlyDownloading((prev) => [...prev, cityItem.id]);

    try {
      const asset = Asset.fromModule(cityItem.asset);
      ...
      const destFile = new File(wallpaperDirectory, fileName);
      ...
      setCurrentlyDownloading((prev) => prev.filter((id) => id !== cityItem.id));
    }
  }
        ...
        ListHeaderComponent={
          <View>
            <Stack direction="horizontal" alignItems="center" gap={8} padding={[16, 10]}>
              <Icon size={24} fill={colors.text}>
                <Papicons name="MapPin" />
              </Icon>

              <Typography style={{ flex: 1 }} variant="body1" color="text">Campus</Typography>
            </Stack>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <WallpaperImage
                  item={{
                    id: item.id,
                    url: Image.resolveAssetSource(item.asset).uri,
                    thumbnail: Image.resolveAssetSource(item.asset).uri,
                    credit: item.name,
                    path: { directory: 'wallpapers', name: `${item.id}.jpg` },
                  }}
                  onPress={() => downloadCity(item)}
                  selectedId={currentWallpaper?.id || ''}
                  isDownloading={currentlyDownloading.includes(item.id)}
                />
              )}
            />
          </View>
