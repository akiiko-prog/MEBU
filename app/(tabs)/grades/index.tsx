...
const GradesView: React.FC = () => {
  ...
  // Manager ref to avoid stale closures
  const managerRef = React.useRef(getManager());

  // Obtention des périodes
  const [periods, setPeriods] = useState<Period[]>([]);
  const [currentPeriod, setCurrentPeriod] = useState<Period>();
  const currentPeriodRef = React.useRef(currentPeriod);
  currentPeriodRef.current = currentPeriod;

  const fetchPeriods = useCallback(async (managerToUse: ReturnType<typeof getManager>) => {
    if (currentPeriodRef.current || !managerToUse) { return; }
  ...
  useEffect(() => {
    const unsubscribe = subscribeManagerUpdate((updatedManager) => {
      ...
      if (currentPeriodRef.current) {
        fetchGradesForPeriod(currentPeriodRef.current, updatedManager);
      }
    });
  ...
  const fetchGradesForPeriod = async (period: Period | undefined, managerToUse = managerRef.current) => {
    ...
    if (period && managerToUse) {
      ...
      const fetchedSubjects = grades.modules || grades.subjects;
      setSubjects(fetchedSubjects);
      if (grades.studentOverall && typeof grades.studentOverall.value === 'number' && !isNaN(grades.studentOverall.value)) {
      ...
      // Sync to iOS widget
      const allGrades = fetchedSubjects.flatMap(s =>
        s.subjects ? s.subjects.flatMap(ss => ss.grades || []) : (s.grades || [])
      );
      const validGrades = allGrades.filter(g =>
        g?.studentScore?.value !== undefined && !g.studentScore.disabled && g.givenAt
      );
      const latestGrades = [...validGrades]
        .sort((a, b) => (safeGetTime(b.givenAt)) - (safeGetTime(a.givenAt)))
        .slice(0, 5)
        .map(g => ({
          subject: getSubjectName(g.subjectName ?? ""),
          value: g.studentScore?.value ?? 0,
      ...
      const avg = grades.studentOverall?.value ?? PapillonSubjectAvg(validGrades);
      syncGradesToWidget({ average: avg, latestGrades, averageHistory: [] });
  ...
  const getSubjectById = useCallback((id: string) => {
    // Search top-level subjects first, then nested subjects (for UE modules)
    const direct = subjects.find((subject) => subject.id === id);
    if (direct) return direct;
    for (const subject of subjects) {
      if (subject.subjects) {
        const nested = subject.subjects.find((s) => s.id === id);
  ...
  const sortedSubjects = useMemo(() => {
    ...
    subjectsCopy.forEach((subject) => {
      (subject.grades ?? []).sort((a, b) => (safeGetTime(b.givenAt)) - (safeGetTime(a.givenAt)));
    });
    ...
    switch (sortMethod) {
      ...
      default:
        subjectsCopy.sort((a, b) => {
          ...
          return safeGetTime(bLatestGrade.givenAt) - safeGetTime(aLatestGrade.givenAt);
        });
        break;
    }
  ...
  const sortedGrades = useMemo(() => {
    ...
    gradesCopy.sort((a, b) => (safeGetTime(b.givenAt)) - (safeGetTime(a.givenAt)));
    return gradesCopy;
  }, [grades]);
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return sortedSubjects
      .map((subject) => {
        ...
        let matchingNestedSubjects: Subject[] | undefined = undefined;
        if (subject.subjects) {
          matchingNestedSubjects = subject.subjects
            .map((nestedSubject) => {
              const nestedName = getSubjectName(nestedSubject.name).toLowerCase();
              if (nestedName.includes(lowerSearchText)) {
                return nestedSubject;
              }

              const matchingNestedGrades = (nestedSubject.grades ?? []).filter((grade) => {
                const descMatch = grade.description?.toLowerCase().includes(lowerSearchText);
                const scoreMatch = grade.studentScore?.value?.toString().includes(lowerSearchText);
                return descMatch || scoreMatch;
              });

              if (matchingNestedGrades.length > 0) {
                return {
                  ...nestedSubject,
                  grades: matchingNestedGrades,
                };
              }

              return null;
            })
            .filter((ns): ns is Subject => ns !== null);
        }
        ...
        const hasMatchingNestedSubjects = matchingNestedSubjects && matchingNestedSubjects.length > 0;

        if (matchingGrades.length > 0 || hasMatchingNestedSubjects) {
          return {
            ...subject,
            grades: matchingGrades.length > 0 ? matchingGrades : (hasMatchingNestedSubjects ? subject.grades : []),
            ...(subject.subjects ? { subjects: matchingNestedSubjects } : {}),
          };
        }
          ...
          <LegendList
            ...
            renderItem={({ item: grade }) =>
              <CompactGrade
                ...
                date={new Date(grade.givenAt ?? Date.now())}
  ...
  ) : null), [sortedGrades, searchText, grades, serviceRank, colors.primary, getSubjectById, getAvgInfluence, getAvgClassInfluence, navigation]);
