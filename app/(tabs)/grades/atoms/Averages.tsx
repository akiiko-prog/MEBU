              ...
              onPressAction={({ nativeEvent }) => {
                ...
                if (actionId.startsWith("open:")) {
                  if (actionId === "open:more") {
                    router.push({ pathname: "/(tabs)/grades/modals/AboutAverages" });
                  }
                }
            ...
            <Dynamic animated key={"dateSource:" + (isRealAverage ? "real" : "estimated")}>
              <Typography color="secondary" style={{ marginTop: 1 }}>
                {isRealAverage ? "par l'établissement" :
                  t("Grades_Avg_Estimated") + " " + (shownDate instanceof Date && !isNaN(shownDate.getTime()) ? shownDate.toLocaleDateString(undefined, {
                    day: "numeric",
                    month: "short",
                    year: "numeric"
                  }) : "Unknown Date")}
