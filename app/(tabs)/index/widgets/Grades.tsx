...
const GradesWidget = (
  ...
  try {
    ...
    const getAverageHistory = useCallback((grades: SharedGrade[]) => {
      ...
      const sortedGrades = [...validGrades].sort((a, b) => toTime(a.givenAt) - toTime(b.givenAt));
    ...
    const fetchGradesForPeriod = useCallback(async (period: Period | undefined, managerToUse = manager) => {
      if (period && managerToUse) {
        try {
          ...
          if (grades.studentOverall && typeof grades.studentOverall.value === 'number' && !isNaN(grades.studentOverall.value) && grades.studentOverall.value > 0) {
