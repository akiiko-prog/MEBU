...
import { EarthIcon, Volume2, BellDot } from "lucide-react-native";
...
const PersonalizationSettings = () => {
  ...
  const [selectedSound, setSelectedSound] = React.useState<"on" | "off">(settingsStore.sound || "on");
  ...
  useEffect(() => {
    mutateProperty('personalization', { sound: selectedSound });
  }, [selectedSound]);
    ...
    <>
      ...
      <ScrollView
        contentContainerStyle={{
          padding: 16,
          paddingTop: Platform.OS === "android" ? height + 16 : 16,
          paddingBottom: 40
        }}
      ...
      >
        ...
        <List style={{ marginTop: 15 }}>
          ...
          <Item>
            <Icon size={30}>
              <Volume2 width={25} height={25} stroke="#818181" />
            </Icon>
            <Typography variant={"title"}>{t("Settings_Personalization_Sound_Title")}</Typography>
            <Trailing>
              <Stack bordered={true}
                direction={"horizontal"}
                height={40}
              ...
              >
                <AnimatedPressable
                  onPress={() => setSelectedSound("on")}
                  style={{ overflow: "hidden", height: "100%" }}
                >
                  <Stack style={{ overflow: "hidden", paddingHorizontal: 15, height: "100%" }}
                    hAlign={"center"}
                    vAlign={"center"}
                    backgroundColor={selectedSound === "on" ? selectedColor : "transparent"}
                    radius={20}
                  >
                    <Papicons name={"check"}
                      opacity={selectedSound === "on" ? 1 : 0.7}
                      color={selectedSound === "on" ? "#FFF" : theme.colors.text}
                    />
                  </Stack>
                </AnimatedPressable>

                <AnimatedPressable
                  onPress={() => setSelectedSound("off")}
                  style={{ overflow: "hidden", height: "100%" }}
                >
                  <Stack style={{ overflow: "hidden", paddingHorizontal: 15, height: "100%" }}
                    hAlign={"center"}
                    vAlign={"center"}
                    backgroundColor={selectedSound === "off" ? selectedColor : "transparent"}
                    radius={20}
                  >
                    <Papicons name={"cross"}
                      opacity={selectedSound === "off" ? 1 : 0.7}
                      color={selectedSound === "off" ? "#FFF" : theme.colors.text}
                    />
                  </Stack>
                </AnimatedPressable>
          ...
          */}
          ...
          >
            ...
            <Typography variant={"title"}>{t("Settings_Personalization_Background_Title")}</Typography>
            <Typography variant={"caption"}
              color={"secondary"}
            >{t("Settings_Personalization_Background_Description")}</Typography>
          </Item>
        </List>
        ...
        <List>
          ...
          */}
          ...
          >
            <Icon size={30}>
              <BellDot width={25} height={25} stroke="#818181" />
            </Icon>
            <Typography variant={"title"}>{t("Settings_Notification_Title")}</Typography>
            <Typography variant={"caption"}
              color={"secondary"}
            >{t("Settings_Notification_Description")}</Typography>
            <Trailing>
              <Icon>
                <Papicons name="ChevronRight" color="#818181" />
              </Icon>
      ...
      <NativeHeaderSide side="Left">
        <HeaderBackButton
          tintColor={runsIOS26 ? theme.colors.text : theme.colors.primary}
          onPress={() => {
            if (router.canGoBack()) {
              router.back();
            } else {
