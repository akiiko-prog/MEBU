...
import { AccountsStorage, Auth, Services, Subject } from "./types";
...
export const useAccountStore = create<AccountsStorage>()(
  persist(
    (set, get) => ({
      ...
      setServiceCredentials: (serviceId: Services, hasCredentials: boolean, accountId?: string) => {
        set({
          accounts: get().accounts.map((account) => {
            const targetAccountId = accountId ?? get().lastUsedAccount;
            const isTarget = account.id === targetAccountId;

            if (isTarget) {
              const existingServiceIndex = account.services.findIndex(s => s.serviceId === serviceId);
              if (existingServiceIndex !== -1) {
                const updatedServices = [...account.services];
                updatedServices[existingServiceIndex] = {
                  ...updatedServices[existingServiceIndex],
                  auth: {
                    ...updatedServices[existingServiceIndex].auth,
                    additionals: {
                      ...updatedServices[existingServiceIndex].auth.additionals,
                      hasStoredCredentials: hasCredentials ? 1 : 0
                    }
                  }
                };
                return { ...account, services: updatedServices };
                ...
                return { ...account, services: [...account.services, newService] };
              }
            }
            return account;
          }),
